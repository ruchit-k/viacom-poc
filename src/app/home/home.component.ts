...
export class HomeComponent {
    ...
    constructor(datetimeAdapter: DatetimeAdapter<FdDate>, private HomeApiService: HomeApiService) {
    }

    ngOnInit(): void {
      this.HomeApiService.getData()
        .then(response => {
          this.source = response.data.entries.slice(0,10);
      //console.log(this.source.slice(0,10))
        })
        .catch(error => {
          console.log(error);
        });
...
export class TableDataProviderExample extends TableDataProvider<ExampleItem> {
    override items: ExampleItem[] = [];
    override totalItems = 0;

    constructor(public override dateTimeAdapter: DatetimeAdapter<FdDate>) {
        super();
    }
